...
use mixctl_core::config_sections::AppletConfig;
...
fn fetch_applet_config() -> AppletConfig {
...
fn main() -> glib::ExitCode {
    let config = fetch_applet_config();
...
fn build_ui(app: &adw::Application, config: &AppletConfig) {
    ...
    let open_cmd = config.open_ui_command.clone();
    open_button.connect_clicked(move |_| {
        std::process::Command::new(&open_cmd).spawn().ok();
    });
